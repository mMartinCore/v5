...
$product = Leave::where(DB::raw("(DATE_FORMAT(end_date,'%Y'))"), date('Y'))->get();




$checkLeaveStatus = Leave::where('officer_id', Session::get('officer_id') )->get();
...
$chart = Charts::database($products, 'bar', 'highcharts')
          ...
          foreach ( $checkLeaveStatus as $key => $value) {
            //print_r($value);
        //    $product_list[] = ['status_id' => $value->status_id, 'id' => $value->id, 'end_date' => $value->end_date];
        $product_list = ['status_id' => $value->status_id];
        $product_listx[] = [ 'location' => $value->locattion];
*/
        ...
        return view('charts',compact( 'pie_chart', 'line_chart', 'areaspline_chart', 'percentage_chart', 'geo_chart', 'area_chart', 'donut_chart'));
    }
}
